    ...
    >
      ...
      <el-table-column prop="" label="原始需求" width="200px" fixed="left">
        <template #default="{ row }">
          <span v-if="typeof row.original_demand === 'object'">
            {{ row.original_demand.join(" ") }}</span
          >
          <span v-if="typeof row.original_demand === 'string'">
            {{ row.original_demand }}</span
          >
        </template>
      </el-table-column>
      ...
      <el-table-column
        label="结果"
        class-name="column_result column_result_first"
        width="100px"
        align="center"
      >
        <template #default="{ row }">
...
const filterTableList = (status: number) => {
  ...
  switch (status) {
    ...
    case 1:
    case 2:
      tableList.value = data.filter(
        (item: BomItem) => item.matchedIcDatas?.[0]?.status === status
      );
      break;
    case 3:
      tableList.value = data.filter(
        (item: BomItem) => item.matchedIcDatas?.length === 0
      );
      break;
    case 4:
...
watch(
  () => [bomParserStore.currentTabLabel, bomParserStore.percentage],
  () => {
